class Player {
	constructor() {
		...
		this.sprite.body.setSize(20, 24, 6, 8);
	...
	update() {
		...
		if (!this.dead && !this.exiting) {
			...
			if (onFloor || onWall) {
				if (slowingDown && !onWall) {
					this.sprite.body.drag.x = C.PLAYER_SLOWDOWN_DRAG;
				} else {
					this.sprite.body.drag.x = C.PLAYER_DRAG;
				}
			} else {
		...
		//game.phaser.debug.geom(new Phaser.Rectangle(this.sprite.body.x, this.sprite.body.y, this.sprite.body.width, this.sprite.body.height), "#FF0000");
	}

	render() {
		if (DEBUG) {
			game.phaser.debug.body(this.sprite);
		}
	}
